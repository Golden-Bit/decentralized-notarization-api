...
from app.utils import simulate_transaction
...
def scenario1_notarize_document(doc: DocumentToNotarizeScenario1, background_tasks: BackgroundTasks):
    ...
    **Esempio di richiesta**:
    ...
    ```
    """
    # Verifica che tutte le blockchain richieste siano "algo"
    validate_blockchains(doc.selected_chain)

    # Salva il file e integra i metadati, ottenendo informazioni aggiuntive
    ...
    background_tasks.add_task(simulate_transaction,
                              doc.storage_id,
                              doc.file_name)
    ...
    return NotarizationResponse(
        ...
        message=(
            f"Documento '{doc.file_name}' in attesa di validaizoni. "
...
def scenario1_query_document_status(query: QueryNotarizationScenario1):
    ...
    **Esempio di richiesta**:
    ...
    # Restituisce il dizionario dei metadati (tutti i campi salvati)
